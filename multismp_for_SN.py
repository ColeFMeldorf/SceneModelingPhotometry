...
class MultiDetection(Detection):
	...
	def __init__(self, ra, dec, expnum, ccdnum, band, color=0.61, name="", color_bg = 0.61, color_bg_iz = None):
		"""
		Constructor class
		"""
		self.ra = ra
	...
	def constructPSFs(
		...
		for i in self.exposures:
			...
			psf_matrix.append(
				sp.csr_matrix(
					construct_psf_background(ra_grid, dec_grid, wcs, self.psf[i['EXPNUM']], x_cen, y_cen, size, flatten=True, color=self.color_bg)))
		print("PSF matrix")
...
#Need to set source prior mean, weights, indices as well as background prior mean, weight (designed for stars), set mean and weight to zero for SN
	...
	def minimizeChisq(
		...
		self.shifts = x_sol
		return x_sol
# Added command to invert cov matrix 
	def minimizeChisqPerStamp(
		self, x_init, size=30, stamp_inds = np.array([0, 10]), source_prior_mean = 0, source_prior_weight = 1, source_prior_inds = np.array([25, 50]), bg_prior_mean = 0, bg_prior_weight = 1, err = True, background=False, method="Powell"
	):
		from scipy.optimize import minimize
		...
		for i in self.exposures[self.stamp_inds[0]:self.stamp_inds[1]][self.exposures["DETECTED"][self.stamp_inds[0]:self.stamp_inds[1]]]:
			try:
				self.source_matrix[j] = construct_psf_source(
		...
		self.constructDesignMatrix(size, background)
		self.solvePhotometry(True, err, source_prior_mean, source_prior_weight, source_prior_inds, bg_prior_mean, bg_prior_weight)
		return x_current
