...
class MultiDetection(Detection):
	...
	def __init__(self, ra, dec, expnum, ccdnum, band, color = 0.61, color_iz = 0.2, name="", color_bg = 0.61, color_bg_iz = 0.2):
		"""
		Constructor class
		"""
		self.ra = ra
		...
		self.band = band
		self.color = color
		self.color_iz = color_iz
		self.name = name
		self.color_bg = color_bg
		self.color_bg_iz = color_bg_iz
	...
	def constructPSFs(
		...
		for i in self.exposures:
			...
			psf_matrix.append(
				sp.csr_matrix(
					construct_psf_background_z(ra_grid, dec_grid, wcs, self.psf[i['EXPNUM']], x_cen, y_cen, size, flatten=True, color=self.color_bg, iz_color = self.color_bg_iz)))
		print("PSF matrix")
			...
			try:
				self.source_matrix.append(
					construct_psf_source_z(
						...
						iz_color = self.color_iz[i['EXPNUM']]
					)
				)
			except (OSError):
					print(i['EXPNUM'])
...
#Updating updateDesignMatrix as well (getting rid of self.ntot - self.ndet + index), 240516: 
	def updateDesignMatrix(self, size, index, source_prior_mean = 0, source_prior_weight = 1, source_prior_inds = np.array([25, 50]), bg_prior_mean = 0, bg_prior_weight = 1):
		self.psf_zeros[index * size * size : (index + 1) * size * size, index] = self.source_matrix[index]
#		self.psf_zeros[(self.ntot - self.ndet + index) * size * size : (self.ntot - self.ndet + index + 1) * size * size, index] = self.source_matrix[index]
		self.design = sp.hstack([self.psf_matrix, self.background, self.psf_zeros], dtype="float64")
		prior_matrix = np.ones(self.design.shape[1])
		prior_matrix[self.psf_matrix.shape[1]:] = 0
		source_ind_st = self.psf_matrix.shape[1] + self.ndet + source_prior_inds[0]
		source_ind_end = source_ind_st + source_prior_inds[1]
		prior_matrix[source_ind_st:source_ind_end] = 1
		self.design = sp.vstack([self.design, np.diag(prior_matrix)], dtype="float64")
#Need to set source prior mean, weights, indices as well as background prior mean, weight (designed for stars), set mean and weight to zero for SN
	def solvePhotometry(self, res=True, err=True, source_prior_mean = 0, source_prior_weight = 1, source_prior_inds = np.array([25, 50]), bg_prior_mean = 0, bg_prior_weight = 1):
		"""
		...
		"""
		self.source_prior_mean = source_prior_mean
		self.source_prior_weight = source_prior_weight
		self.bg_prior_mean = bg_prior_mean
		self.bg_prior_weight = bg_prior_weight
		prior_matrix = np.ones(self.design.shape[1])
		prior_matrix[self.psf_matrix.shape[1]:] = 0
		source_ind_st = self.psf_matrix.shape[1] + self.ndet + source_prior_inds[0]
		source_ind_end = source_ind_st + source_prior_inds[1]
		prior_matrix[source_ind_st:source_ind_end] = 1
		design_with_prior = sp.vstack([self.design, np.diag(prior_matrix)])
		inv_weight_prior_array = np.ones(self.design.shape[1]) * bg_prior_weight
		inv_weight_prior_array[self.psf_matrix.shape[1]:] = 0
		inv_weight_prior_array[source_ind_st:source_ind_end] = source_prior_weight
		inv_weight_prior = np.concatenate([self.invwgt, inv_weight_prior_array])
		self.diag = sp.diags(np.sqrt(inv_weight_prior))
		print("Product")
		#prod = self.diag.dot(self.design)
		prod = self.diag.dot(design_with_prior)
		#self.target = np.concatenate(prior_mean_vals, self.image * np.sqrt(self.invwgt)) # Concatenate zeros for the prior elements
		target_array = np.ones(self.design.shape[1]) * bg_prior_mean
		target_array[self.psf_matrix.shape[1]:] = 0
		target_array[source_ind_st:source_ind_end] = source_prior_mean
		self.target = np.concatenate([self.image, target_array]) * np.sqrt(inv_weight_prior)
		print("Solving")
	...
	def minimizeChisqPerStamp(
		self, x_init, size=30, stamp_inds = np.array([0, 10]), source_prior_mean = 0, source_prior_weight = 1, source_prior_inds = np.array([25, 50]), bg_prior_mean = 0, bg_prior_weight = 1, background=False, method="Powell"
	):
		from scipy.optimize import minimize
		self.stamp_inds = stamp_inds
		x_current = x_init
		for i in range(self.stamp_inds[0], self.stamp_inds[1]):
			...
			x_init[2*i:2*i+2],
			method=method,
			args=(self, i, size, source_prior_mean, source_prior_weight, source_prior_inds, bg_prior_mean, bg_prior_weight),
			options={"xtol": 0.01},
		)
		...
		#self.source_matrix = []
		j = self.stamp_inds[0]
		for i in self.exposures[self.stamp_inds[0]:self.stamp_inds[1]][self.exposures["DETECTED"][self.stamp_inds[0]:self.stamp_inds[1]]]:
			try:
				self.source_matrix[j] = construct_psf_source_z(
		...
		self.constructDesignMatrix(size, background)
		self.solvePhotometry(True, True, source_prior_mean, source_prior_weight, source_prior_inds, bg_prior_mean, bg_prior_weight)
		return x_current
...
def chi2_perstamp(x, detection, stamp, size = 30, source_prior_mean = 0, source_prior_weight = 1, source_prior_inds = np.array([25, 50]), bg_prior_mean = 0, bg_prior_weight = 1):
	expnum = detection.exposures[detection.exposures['DETECTED']][stamp]['EXPNUM']
	detection.source_matrix[stamp] = construct_psf_source_z(detection.x_cen[expnum] + x[0], detection.y_cen[expnum] + x[1], detection.psf[expnum], size, detection.x_cen[expnum], detection.y_cen[expnum], detection.color_iz[expnum])
	detection.updateDesignMatrix(size,stamp, source_prior_mean, source_prior_weight, source_prior_inds, bg_prior_mean, bg_prior_weight)
